...
CREATE TABLE `room_basic` (
    ...
    `user_uid` BIGINT UNSIGNED NOT NULL COMMENT '创建者ID',
...
CREATE TABLE `message_basic` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_uid` BIGINT UNSIGNED NOT NULL COMMENT '接收者ID',
  `sender_uid` BIGINT UNSIGNED NOT NULL COMMENT '发送者ID',
  `room_uid` VARCHAR(50) NOT NULL COMMENT '房间ID',
  `session_type` TINYINT NOT NULL COMMENT '会话类型 1:群聊 2:私聊',
  `content` LONGTEXT NOT NULL COMMENT '聊天内容',
  `content_type` TINYINT NOT NULL COMMENT '聊天内容类型 1:文本 2:图片 3:视频 4:音频 5:文件',
